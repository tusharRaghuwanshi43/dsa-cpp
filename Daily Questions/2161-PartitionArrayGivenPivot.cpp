...
// Approach 1 -> Creating Different vector to store smaller, medium and larger elements.
class Solution1
{
public:
    vector<int> pivotArray(vector<int> &nums, int pivot)
    {
        ...
        ans = smaller;
        ans.insert(ans.end(), medium.begin(), medium.end());
        ans.insert(ans.end(), larger.begin(), larger.end());
        return ans;
    }
};

// Approach 2 -> Looping three different loops for three different conditions
class Solution
{
public:
    vector<int> pivotArray(vector<int> &nums, int pivot)
